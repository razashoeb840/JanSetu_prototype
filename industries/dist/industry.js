...
function initUI() {
  ...
  if (document.getElementById('sidebarName')) document.getElementById('sidebarName').textContent = currentUser.name;
  if (document.getElementById('topbarName')) document.getElementById('topbarName').textContent = currentUser.name;
  if (document.getElementById('welcomeName')) document.getElementById('welcomeName').textContent = `Welcome, ${currentUser.name.split(' ')[0]}!`;
...
function showSection(section) {
  ...
  const titles = {
    ...
    heatmap: ['Pan-India & District CSR Heatmap', 'Intelligence / Heatmap'],
  ...
  if (document.getElementById('pageBreadcrumb')) document.getElementById('pageBreadcrumb').textContent = crumb;
...
// ── Pan-India Heatmap Controller for Industry ─────────────────────────────
let _industryHeatmapInstance = null;
function initIndustryHeatmap() {
  if (_industryHeatmapInstance) {
    _industryHeatmapInstance.invalidateSize();
    return;
  }
  if (typeof PanIndiaHeatmap !== 'undefined') {
    _industryHeatmapInstance = new PanIndiaHeatmap({
      containerId: 'industry-india-map',
      stateSelectId: 'industryStateSelect',
      categoryFilterId: 'industryMapCategoryFilter',
      detailPanelId: 'industryDistrictDetailPanel',
      role: 'industry'
    });
    window.panIndiaMapInstance = _industryHeatmapInstance;
  } else {
    console.warn('PanIndiaHeatmap library not loaded yet');
  }
}
window.initIndustryHeatmap = initIndustryHeatmap;
...
function enrichChallengeData(c) {
  ...
  const hasAssignedHei = !!(c.assignedUniversity && (c.assignedUniversity.name || c.assignedUniversity.shortName));
  const defaultHei = hasAssignedHei
    ? (c.assignedUniversity.name || c.assignedUniversity.shortName)
    : 'Open for University Assignment';
  const defaultDept = hasAssignedHei
    ? (c.assignedUniversity.department || 'Applied Technology & Innovation Lab')
    : 'Pending Academic Taskforce Allocation';
  const defaultFaculty = hasAssignedHei
    ? (c.assignedUniversity.facultyMentor || 'Faculty Taskforce Lead')
    : 'Awaiting Faculty Selection';

  // Real stage mapping from status
  let defaultStage = 'Problem Submitted';
  let defaultProgress = 15;
  const rawStatus = (c.status || '').toLowerCase();
  if (rawStatus === 'resolved' || rawStatus === 'closed') {
    defaultStage = 'Deployment';
    defaultProgress = 100;
  } else if (rawStatus === 'testing') {
    defaultStage = 'Field Testing';
    defaultProgress = 85;
  } else if (c.industryCollaborators && c.industryCollaborators.length > 0) {
    defaultStage = 'Industry Collaboration';
    defaultProgress = 60;
  } else if (rawStatus === 'in_progress') {
    defaultStage = 'Solution Development';
    defaultProgress = 45;
  } else if (rawStatus === 'assigned' || hasAssignedHei) {
    defaultStage = 'University Assigned';
    defaultProgress = 30;
  } else if (rawStatus === 'validated') {
    defaultStage = 'Validated';
    defaultProgress = 20;
  }
...
function renderTimelineSteps(currentStage, challenge = null) {
  const c = challenge || currentSelectedChallenge || {};
  const rawStatus = (c.status || '').toLowerCase();

  let activeIdx = 0;
  if (rawStatus === 'resolved' || rawStatus === 'closed') activeIdx = 7;
  else if (rawStatus === 'testing') activeIdx = 6;
  else if (Array.isArray(c.industryCollaborators) && c.industryCollaborators.length > 0) activeIdx = 4;
  else if (rawStatus === 'in_progress') activeIdx = 3;
  else if (rawStatus === 'assigned' || c.assignedUniversity) activeIdx = 2;
  else if (rawStatus === 'validated') activeIdx = 1;
  else activeIdx = 0;

  const history = Array.isArray(c.statusHistory) ? c.statusHistory : [];
  const subEvent = history.find(h => h.status === 'submitted');
  const valEvent = history.find(h => h.status === 'validated');
  const asgEvent = history.find(h => h.status === 'assigned');
  const devEvent = history.find(h => h.status === 'in_progress');
  const tstEvent = history.find(h => h.status === 'testing');
  const resEvent = history.find(h => h.status === 'resolved' || h.status === 'closed');
  const indEvent = Array.isArray(c.industryCollaborators) && c.industryCollaborators.length > 0 ? c.industryCollaborators[0] : null;

  const subDate = subEvent ? formatTimelineDate(subEvent.changedAt) : (c.createdAt ? formatTimelineDate(c.createdAt) : '--');
  const valDate = valEvent ? formatTimelineDate(valEvent.changedAt) : (activeIdx >= 1 && c.updatedAt ? formatTimelineDate(c.updatedAt) : '--');
  const asgDate = asgEvent ? formatTimelineDate(asgEvent.changedAt) : (c.assignedAt ? formatTimelineDate(c.assignedAt) : (activeIdx >= 2 && c.updatedAt ? formatTimelineDate(c.updatedAt) : '--'));
  const devDate = devEvent ? formatTimelineDate(devEvent.changedAt) : (activeIdx >= 3 && c.updatedAt ? formatTimelineDate(c.updatedAt) : '--');
  const indDate = indEvent ? formatTimelineDate(indEvent.joinedAt) : (activeIdx >= 4 && c.updatedAt ? formatTimelineDate(c.updatedAt) : '--');
  const prtDate = activeIdx >= 5 && c.updatedAt ? formatTimelineDate(c.updatedAt) : '--';
  const tstDate = tstEvent ? formatTimelineDate(tstEvent.changedAt) : (activeIdx >= 6 && c.updatedAt ? formatTimelineDate(c.updatedAt) : '--');
  const depDate = resEvent ? formatTimelineDate(resEvent.changedAt) : (c.resolvedAt ? formatTimelineDate(c.resolvedAt) : '--');
  ...
  const stageNotes = [
    'Citizen Verified',
    activeIdx >= 1 ? 'Govt Approved' : 'Pending',
    activeIdx >= 2 ? (c.assignedUniversity?.shortName || 'HEI Assigned') : 'Pending',
    activeIdx >= 3 ? 'R&D In Progress' : 'Pending',
    activeIdx >= 4 ? 'CSR Partner Active' : 'Open for CSR',
    activeIdx >= 5 ? 'Working Prototype' : 'Pending',
    activeIdx >= 6 ? 'Field Testing' : 'Pending',
    activeIdx >= 7 ? 'Public Rollout' : 'Pending'
  ];

  let html = '<div class="timeline-milestones-row">';
  LIFECYCLE_STAGES.forEach((st, idx) => {
    const isCompleted = idx < activeIdx;
    const isActive = idx === activeIdx;
    const bubbleClass = isCompleted ? 'completed' : (isActive ? 'current active' : '');
    const bubbleContent = isCompleted ? '✓' : (idx + 1);
    const dateText = stageDates[idx] || '--';
    const noteText = stageNotes[idx] || '';

    html += `
      <div class="timeline-step-item">
        <div class="timeline-dot-bubble ${bubbleClass}">
          ${bubbleContent}
        </div>
        <div class="timeline-step-label ${isActive ? 'active' : ''}">
          ${st}
        </div>
    ...
    if (idx < LIFECYCLE_STAGES.length - 1) {
      ...
      html += `
        <div class="timeline-connecting-line ${isLineActive ? 'active-line' : ''}"></div>
      `;
    }
  });
    ...
    <div style="display:flex;flex-direction:column;gap:24px">
      ...
      <div class="card">
        ...
        <div class="card-body" style="padding:22px 24px">
          ...
          ` : `
            <div style="background:#f8fafc;border:1.5px dashed #cbd5e1;border-radius:var(--radius-lg);padding:20px 24px;display:flex;align-items:center;gap:16px;flex-wrap:wrap">
              <div style="font-size:32px">🏛️</div>
              <div style="flex:1;min-width:260px">
                <div style="font-size:16px;font-weight:750;color:var(--gray-900)">Open for University &amp; R&amp;D Assignment</div>
                <div style="font-size:13px;color:var(--gray-600);margin-top:4px;line-height:1.5">
                  This civic challenge has been verified by the Government/Administration and is currently in queue for academic taskforce allocation across accredited HEIs in Jharkhand.
                </div>
              </div>
              <span class="badge badge-amber" style="font-size:12px;font-weight:750;padding:6px 12px">⚡ Allocation In Queue</span>
            </div>
          `}
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div class="card-body" style="padding:24px 24px 18px">
          <div class="timeline-steps">
            ${renderTimelineSteps(c.projectStage, c)}
          </div>
        </div>
      </div>
  ...
  milestones: [
    {
      ...
      status: 'Completed',
      startedDate: '01 Sep 2026',
      expectedDate: '05 Sep 2026',
      description: 'On-ground surveys with local farmers to identify water scarcity patterns and irrigation bottlenecks.',
      progress: 100
    },
    {
      id: 'M2',
      ...
      status: 'Completed',
      startedDate: '05 Sep 2026',
      expectedDate: '07 Sep 2026',
      description: 'Literature review on soil permittivity, LoRaWAN range in rural terrain, and solar power budgeting.',
      progress: 100
    },
    {
      id: 'M3',
      ...
      status: 'Completed',
      startedDate: '07 Sep 2026',
      expectedDate: '08 Sep 2026',
      description: 'Circuit schematic drafting, microcontroller selection, and industrial casing CAD models.',
      progress: 100
    },
    {
      id: 'M4',
      ...
      status: 'In Progress',
      startedDate: '08 Sep 2026',
      expectedDate: '15 Sep 2026',
      description: 'Assembling v1.0 prototype units, calibrating capacitive moisture probes, and telemetry dashboards.',
      progress: 70
    },
    {
      id: 'M5',
      ...
      status: 'Pending',
      startedDate: 'Pending',
      expectedDate: 'Est. Oct 2026',
      description: 'Deploying pilot test beds across regional farm clusters for live ground validation.',
      progress: 0
    },
    {
      id: 'M6',
      ...
      status: 'Pending',
      startedDate: 'Pending',
      expectedDate: 'Est. Nov 2026',
      description: 'Structured community interviews and usability evaluations.',
      progress: 0
    },
    {
      id: 'M7',
      ...
      startedDate: 'Pending',
      expectedDate: 'Est. Nov 2026',
      description: 'Handover of production-ready units with extension team training.',
      progress: 0
    },
    {
      id: 'M8',
      ...
      status: 'Pending',
      startedDate: 'Pending',
      expectedDate: 'Est. Dec 2026',
      description: 'Auditing resource savings, community benefit metrics, and CSR sustainability reports.',
      progress: 0
    }
  ],
