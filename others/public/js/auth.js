...
window.quickLogin = async (email, password) => {
  const form = document.getElementById('loginForm');
  if (form && document.getElementById('email') && document.getElementById('password')) {
...
function initRegister() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (res.success) {
        Auth.setAuth(res.token, res.user);
        const idMsg = res.user.universityIdString 
          ? ` Your University ID is: ${res.user.universityIdString}.` 
          : (res.user.citizenId ? ` Your Citizen ID is: ${res.user.citizenId}.` : '');
        Toast.success('Account created!' + idMsg, 'Welcome to JanSetu!');
        setTimeout(() => Auth.redirectToDashboard(res.user.role), 1500);
      }
    } catch (error) {
